...
class PSOEnvironment():
    ...
    def print_best_audio(self):
        print(
            "Current decoded word: " + self.ds.stt(self.gbest_position.astype(np.int16), 16000) + "\t" + "Cost: " + str(
    ...
    def update(self, print_toggle):
        ...
        for particle in self.particles:
            particle.velocity = self.w * particle.velocity + (self.c1 * np.random.random()) * (
                        particle.pbest_position - particle.position) + (self.c2 * np.random.random()) * (
...
def main():
    ...
    with tf.Session() as sess:
        ...
        wav.write(out_wav_file, 16000, pso_environment.gbest_position.astype(np.int16))
